...
import { ArrowLeft, CreditCard, Download, Loader2, Printer } from 'lucide-react';
...
export default function InvoiceDetailPage() {
    ...
    const formatDate = (dateString: string | Date | undefined) => {
        ...
        try { return format(new Date(dateString), 'dd/MM/yyyy'); } catch { return '-'; }
    };

    const formatDateTime = (dateString: string | Date | undefined) => {
        if (!dateString) return '-';
        try { return format(new Date(dateString), 'dd/MM/yyyy HH:mm'); } catch { return '-'; }
    };
    ...
    const handleExportPDF = async () => {
        ...
        try {
            ...
            if (pdfHeight + 10 > pageHeight) {
                let remainH = pdfHeight;
                let position = 10;
                while (remainH > 0) {
                    pdf.addImage(imgData, 'PNG', 0, position, pdfWidth, pdfHeight);
                    remainH -= (pageHeight - 10);
                    if (remainH > 0) { pdf.addPage(); position = -(pdfHeight - remainH); }
                }
            } else {
    ...
    const handlePrint = () => {
        ...
        printWindow.document.write(`
            <html><head><title>In hóa đơn - ${invoice.invoiceNumber}</title>
            <style>${cssText}
            @import url('https://fonts.googleapis.com/css2?family=JetBrains+Mono:wght@400;500;700&display=swap');
            body { font-family: 'JetBrains Mono', 'Courier New', monospace; margin: 0; padding: 20px; background: white; }
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-3xl font-bold tracking-tight">
                            {t('invoices.detailTitle') || 'Chi tiết hóa đơn'}
                        </h1>
                        <p className="text-muted-foreground font-mono text-sm">{invoice.invoiceNumber}</p>
                    </div>
                </div>
                <div className="flex items-center gap-2">
                    ...
                    {invoice.status !== 'PAID' && invoice.status !== 'CANCELLED' && (
                        <Button size="sm" className="bg-emerald-600 hover:bg-emerald-700" onClick={() => setIsPaymentOpen(true)}>
                            <CreditCard className="mr-2 h-4 w-4" />
                            {t('payments.record') || 'Ghi nhận thanh toán'}
                        </Button>
                    )}
                </div>
            </div>
            ...
            <div className="flex justify-center">
                <div
                    ref={receiptRef}
                    className="shadow-lg w-full max-w-[420px]"
                    style={{
                        fontFamily: "'JetBrains Mono', 'Courier New', monospace",
                        fontSize: '11px', lineHeight: '1.5',
                        padding: '24px 20px', backgroundColor: '#ffffff', color: '#0f172a',
                    }}
                >
                    {/* Header */}
                    ...
                    {/* Status */}
                    <div className="text-center py-2 px-4 border-2 border-dashed rounded mb-4" style={{ borderColor: statusColor }}>
                        <span className="font-bold text-xs" style={{ color: statusColor }}>{getStatusText(invoice.status)}</span>
                    </div>
                    ...
                    {/* Customer Info */}
                    <div className="mb-4">
                        <div className="font-bold text-[10px] uppercase tracking-wider mb-2 border-b pb-1" style={{ color: '#475569' }}>Thông tin khách hàng</div>
                    ...
                    {isShortTerm && (
                        <>
                            <div className="mb-4">
                                <div className="font-bold text-[10px] uppercase tracking-wider mb-2 border-b pb-1" style={{ color: '#475569' }}>Thời gian lưu trú</div>
                                <div className="space-y-1">
                                    <div className="flex justify-between">
                    ...
                    {/* Items */}
                    <div className="mb-4">
                        <div className="font-bold text-[10px] uppercase tracking-wider mb-2 border-b pb-1" style={{ color: '#475569' }}>Chi tiết</div>
                        ...
                        {!isShortTerm && invoice.electricityUsed > 0 && (
                            <>
                                ...
                                <div className="text-[10px] uppercase tracking-wider mt-2 mb-1" style={{ color: '#64748b' }}>Điện nước</div>
                                <div className="py-1">
                                    <div className="flex justify-between">
                                        <span>Tiền điện</span>
                        ...
                        {invoice.serviceCharges?.length > 0 && (
                            <>
                                <div className="border-b border-dotted my-1" style={{ borderColor: '#e2e8f0' }} />
                                <div className="text-[10px] uppercase tracking-wider mt-2 mb-1" style={{ color: '#64748b' }}>Dịch vụ</div>
                        ...
                        {invoice.adjustments?.length > 0 && (
                            <>
                                <div className="border-b border-dotted my-1" style={{ borderColor: '#e2e8f0' }} />
                                <div className="text-[10px] uppercase tracking-wider mt-2 mb-1" style={{ color: '#64748b' }}>Điều chỉnh</div>
                    ...
                    {/* Payment */}
                    <div className="mb-4">
                        <div className="font-bold text-[10px] uppercase tracking-wider mb-2 border-b pb-1" style={{ color: '#475569' }}>Thanh toán</div>
                        <div className="space-y-1">
                            <div className="flex justify-between">
                    ...
                    {/* Tear-off */}
                    <div className="mt-6 -mx-5 -mb-6 h-4"
                        style={{ background: 'repeating-linear-gradient(90deg, transparent, transparent 8px, #f1f5f9 8px, #f1f5f9 16px)' }}
                    />
                </div>
            </div>
